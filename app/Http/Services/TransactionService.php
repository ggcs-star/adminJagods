...
class TransactionService
{
  ...
  protected function transaction(
    ...
    $transaction->source_balance_id      = $sourceBalanceId;
    $transaction->destination_balance_id = $destinationBalanceId;
    $transaction->amount                 = $amount;
    $transaction->status                 = 1;

    $transaction->creator_type           = User::class;
    $transaction->editor_type            = User::class;
    $transaction->creator_id             = $userId;
    $transaction->editor_id              = $userId;

    $transaction->meta                   = [];
    $transaction->invoice_id             = 0;
    $transaction->order_id               = 0;
    $transaction->restaurant_id          = 0;
