        ...
        <div class="col-12">
            <div class="db-card">
                ...
                <div class="db-card-body">
                    <form action="{{ route('admin.cuisine.update', $cuisine) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            ...
                            <div class="form-col-12 sm:form-col-6 md:form-col-4">
                                <label class="db-field-title required" for="restroType">
                                    Restaurant Type
                                </label>
                                ...
                                <div class="db-field-down-arrow">
                                    <select name="restroType" id="restroType"
                                        ...
                                        <option value="{{ \App\Enums\RestroType::VEG }}"
                                            {{ old('restroType', $cuisine->restroType) == \App\Enums\RestroType::VEG ? 'selected' : '' }}>
                                            Veg
                                        </option>

                                        <option value="{{ \App\Enums\RestroType::NON_VEG }}"
                                            {{ old('restroType', $cuisine->restroType) == \App\Enums\RestroType::NON_VEG ? 'selected' : '' }}>
                                            Non Veg
                                        </option>

                                        <option value="{{ \App\Enums\RestroType::VEG_AND_NON_VEG }}"
                                            {{ old('restroType', $cuisine->restroType) == \App\Enums\RestroType::VEG_AND_NON_VEG ? 'selected' : '' }}>
                                            Veg & Non Veg
                                        </option>
                                    </select>
                                </div>
                                    ...
                                    <div class="db-field-down-arrow">
                                        <select name="status"
                                            ...
                                            @foreach (trans('statuses') as $key => $status)
                                                <option value="{{ $key }}"
                                                    {{ old('status', $cuisine->status) == $key ? 'selected' : '' }}>
                                                    {{ $status }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <div class="form-col-12">
                                ...
                                <textarea name="description" class="db-field-control @error('description') invalid @enderror" id="editor"
                                    cols="30" rows="10">{{ old('description', $cuisine->description) }}</textarea>
                                @error('description')
                                    <small class="db-field-alert">{{ $message }}</small>
                                @enderror
                            </div>
